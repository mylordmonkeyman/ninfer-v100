...
A [frozen layer-zero boundary report](https://github.com/mylordmonkeyman/ninfer-v100/actions/runs/36141905854)
shows that position-12 CPU and V100 attention inputs match exactly. Their
recurrent output differs by 1.32e-5 NRMSE, gated output by 0.000266,
attention output by 0.000220, post-attention hyper state by 0.001768,
and BF16 MLP input by 0.001124. The CPU hyper trace was generated by an
earlier profile revision, while the GDN trace uses the current profile;
layer zero precedes PLE, but this mixed-artifact comparison still needs a
same-run confirmation before assigning the intermediate increase to an
engine operation. Fitting the V100 hyper update to its attention output
and four injection scales leaves less than 5e-9 normalized residual per
stream; the analogous CPU reconstruction has residuals of 5e-5 to 3e-4.
The V100 FP32 attention-injection arithmetic is internally consistent at
this boundary. Capture the CPU pre-injection master state and actual gates
before interpreting the CPU residual as a kernel mismatch.
