...
struct PressureCheckpointRecoveryImpact {
    ...
    [[nodiscard]] friend constexpr bool
    operator==(const PressureCheckpointRecoveryImpact& a,
               const PressureCheckpointRecoveryImpact& b) noexcept {
        return a.owner == b.owner && a.checkpoint == b.checkpoint &&
               a.survives == b.survives &&
               a.target_recovery_work.data() == b.target_recovery_work.data() &&
               a.target_recovery_work.size() == b.target_recovery_work.size();
    }
};
