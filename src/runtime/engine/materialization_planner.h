...
private:
    ...
    // 64x64 -> 128-bit product as {high, low}; pairs compare lexicographically.
    [[nodiscard]] static std::pair<std::uint64_t, std::uint64_t>
    multiply_wide(std::uint64_t a, std::uint64_t b) noexcept {
        constexpr std::uint64_t kMask = 0xFFFFFFFFULL;
        const std::uint64_t a_lo = a & kMask, a_hi = a >> 32U, b_lo = b & kMask, b_hi = b >> 32U;
        const std::uint64_t ll = a_lo * b_lo, lh = a_lo * b_hi, hl = a_hi * b_lo, hh = a_hi * b_hi;
        const std::uint64_t mid = (ll >> 32U) + (lh & kMask) + (hl & kMask);
        const std::uint64_t lo  = (mid << 32U) | (ll & kMask);
        const std::uint64_t hi  = hh + (lh >> 32U) + (hl >> 32U) + (mid >> 32U);
        return {hi, lo};
    }
    ...
    [[nodiscard]] static bool construction_option_better(const GuidanceCost& parent,
        ...
        if (a && b) {
            ...
            // delta(cost) * b < delta(prior) * a, without a 128-bit integer type (MSVC has none).
            const auto left  = multiply_wide(delta(cost), b);
            const auto right = multiply_wide(delta(prior), a);
