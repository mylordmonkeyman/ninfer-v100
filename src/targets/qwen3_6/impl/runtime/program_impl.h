...
ProgramImplCore::ProgramImplCore(const LoadedModelData& model_in, const SequencePlanImpl& plan,
    ...
    : model(model_in), device(device_in), capacity(plan.capacity), kv_capacity(plan.kv_capacity),
      ...
      score_logprobs_host(plan.causal_scoring ? std::make_optional<PinnedHostBuffer>(
                                              ...
                                              : std::nullopt),
      ordinary_host(sizeof(qwen3_6::OrdinaryDecodeIngress) +
...
std::optional<AdmissionCandidate> ProgramImplCore::inspect_admission(
    ...
    // Pressure can free capacity, but cannot complete another lane's state transition. Do not
    // offer such an identity to the pressure search: composed alternatives assess capacity and
    // would otherwise appear feasible until seal revalidates this same boundary.
...
runtime::ExecutionTiming ProgramImplCore::resolve_pending_raw(
    ...
    if (!lanes.empty() && lanes.front() < max_concurrency &&
        ...
        }
        {
            ...
            *ordinary_host_egress  = {};
            for (std::uint32_t row = 0; row < batch_size; ++row) {
                ...
                ordinary_host_ingress->next_cache_positions[row] =
                    checked_i32(frontier + 1, "graph representative ordinary next position");
                ...
                ordinary_host_ingress->backend_kv_table_rows[row] = static_cast<std::int32_t>(row);
                ordinary_host_ingress->active_lanes[row] = static_cast<std::int32_t>(row);
                ordinary_host_ingress->valid_columns[row] = 1;
...
ProgramImplCore::decode_ordinary_batch(std::span<const std::uint32_t> lanes,
    ...
    try {
        ...
        submit_range.emplace(nvtx::Name::DecodeOrdinarySubmit, nvtx::Category::Decode,
                             static_cast<std::uint64_t>(lanes.size()));
        // A preceding speculative round may leave committed target features pending. Append
        // them before the width-one target call overwrites that lane's feature buffer.
        if (dflash) {
            std::array<std::uint32_t, kMaximumConcurrency> append_lanes{}, starts{}, counts{};
            std::size_t count = 0;
            for (const std::uint32_t lane : lanes) {
                const SequenceState& sequence = active_sequence(lane);
                if (sequence.dflash_context_frontier > sequence.execution_frontier) {
                    throw std::logic_error("ordinary target round has an advanced draft context");
                }
                if (sequence.dflash_context_frontier < sequence.execution_frontier) {
                    append_lanes[count] = lane;
                    starts[count] = sequence.dflash_context_frontier;
                    counts[count] = sequence.execution_frontier - sequence.dflash_context_frontier;
                    ++count;
                }
            }
            if (count != 0) {
                enqueue_dflash_context_append(std::span(append_lanes.data(), count),
                                               std::span(starts.data(), count),
                                               std::span(counts.data(), count));
                for (std::size_t row = 0; row < count; ++row) {
                    SequenceState& sequence = active_sequence(append_lanes[row]);
                    sequence.dflash_context_frontier = sequence.execution_frontier;
                }
            }
        }
        ...
        for (std::size_t row = 0; row < lanes.size(); ++row) {
            ...
            ordinary_host_ingress->next_cache_positions[row] =
                checked_i32(frontier + 1, "ordinary batch next position");
            ordinary_host_ingress->rope_positions[row] =
            ...
            ordinary_host_ingress->backend_kv_table_rows[row] = sequence.kv->backend
                ? backend_kv_addresses->bound_row(*sequence.kv->backend) : 0;
            ordinary_host_ingress->active_lanes[row] = static_cast<std::int32_t>(lanes[row]);
            ordinary_host_ingress->valid_columns[row] = 1;
            const StateImageSelectors selectors                 = state_selectors(sequence);
        ...
        for (std::size_t row = 0; row < lanes.size(); ++row) {
            ...
            if (speculative_backend == SpeculativeBackend::Mtp) {
                ...
                sequence.dflash_context_frontier = base_E + 1;
            }
            commit_sequence_kv(sequence, sequence.text_kv_valid, backend_kv_valid(sequence));
...
ProgramImplCore::decode_raw(std::span<const std::uint32_t> lanes,
                            ...
                            runtime::ExecutionTiming* failed_timing) {
    if (lanes.empty() || lanes.size() > max_concurrency || budgets.size() != lanes.size()) {
        throw std::invalid_argument("decode batch membership is invalid");
    }
    ...
    if (speculative_backend == SpeculativeBackend::None || target_only) {
