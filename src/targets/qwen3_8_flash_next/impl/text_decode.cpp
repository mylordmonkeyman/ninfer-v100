...
void flash_next_text_decode_core(const TextModelView& model, const Tensor& embedding,
    ...
    const bool fp32_mlp_hyper_input = fp32_hyper_state && fp32_mlp_hyper_output && [] {
    ...
    const bool fp32_mlp_hyper_low_rank = fp32_mlp_hyper_normalized && [] {
        const char* env = std::getenv("NINFER_FLASH_NEXT_FP32_MLP_HYPER_LOW_RANK");
        return env != nullptr && env[0] == '1' && env[1] == '\0';
    }();
    const bool fp32_mlp_hyper_apply = fp32_mlp_hyper_low_rank && [] {
    ...
    const bool fp32_moe_shared_input = fp32_mlp_hyper_apply && [] {
    ...
    for (std::size_t layer = 0; layer < 48; ++layer) {
        ...
        if (fp32_mlp_hyper_low_rank && fp32_mlp_hyper_apply) {
            ...
            flash_next_hyper_prepare(round_ws.hyper_hidden, model.layers[layer].mlp_hyper,
                                     round_ws.hyper_scratch, round_ws.block_input, stream);
        } else if (fp32_mlp_hyper_normalized) {
        ...
        if (model.host_experts.has_value()) {
            ...
            const Tensor* routed_expert_input =
                ...
                nullptr;
#endif
            const Tensor* shared_expert_input =
#if defined(NINFER_VOLTA_BUILD)
                fp32_moe_shared_input ? &round_ws.hyper_scratch.mixed_fp32 : nullptr;
#else
                nullptr;
#endif
