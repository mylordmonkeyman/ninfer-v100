...
void embed_gather_fp8_launch(const Tensor& ids, const Weight& table, Tensor& out,
    ...
    if (d == 5120) {
        if (is_f32_scale) {
            if (T <= 176) {
                launch_fp8<5120, 10, 128, float>(ids, table, out, stream);
            } else {
                launch_fp8<5120, 5, 128, float>(ids, table, out, stream);
            }
        } else {
            if (T <= 176) {
                launch_fp8<5120, 10, 128, __nv_bfloat16>(ids, table, out, stream);
            } else {
                launch_fp8<5120, 5, 128, __nv_bfloat16>(ids, table, out, stream);
            }
        }
    } else if (d == 2560) {
        if (is_f32_scale) {
