...
Geometry validate_recurrent(const Tensor& q, const Tensor& k, const Tensor& v, const Tensor& g,
    ...
    if (ssm_state.dtype != DType::FP32 && ssm_state.dtype != DType::BF16) {
        throw std::invalid_argument("gated_delta_net: ssm_state must be FP32 or BF16");
    }
...
Geometry validate_recurrent_batch_update(const Tensor& q, const Tensor& k, const Tensor& v,
    ...
    if (ssm_states.dtype != DType::FP32 && ssm_states.dtype != DType::BF16) {
        throw std::invalid_argument("gated_delta_net: ssm_states must be FP32 or BF16");
    }
...
void validate_chunked(const Tensor& q, const Tensor& k, const Tensor& v, const Tensor& g,
    ...
    if (ssm_state_in.dtype != DType::FP32 && ssm_state_in.dtype != DType::BF16) {
        throw std::invalid_argument("gated_delta_net: ssm_state_in must be FP32 or BF16");
    }
    if (ssm_state_in.dtype != ssm_state_out.dtype) {
        throw std::invalid_argument("gated_delta_net: ssm_state_in and ssm_state_out dtypes must match");
    }
