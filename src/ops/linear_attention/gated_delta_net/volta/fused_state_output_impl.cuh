...
namespace ninfer::ops::detail::gated_delta_net::volta::fused_detail {

using Dv16Schedule = GdnSchedule<16>;
using Dv16Shared = FusedSharedLayout<16>;

inline constexpr int kDvTile = 16;
inline constexpr int kXyLd = kMacroFp32Ld;
...
static_assert(2 * kChunkSize * kXyLd * sizeof(float) == 4352);
static_assert(kDvTile * kQkHalfLd * sizeof(__half) == 4160);
...
__device__ __forceinline__ BridgePlan finish_cta_range(RangeStats local, float* scratch) {
    ...
    if (tid == 0) {
        RangeStats total{};
        for (int w = 0; w < Dv16Schedule::kWarps; ++w) {
            RangeStats part{
                scratch[w],
                scratch[Dv16Schedule::kWarps + w],
            };
...
__device__ __forceinline__ void load_state(float (&h)[4][4], const FusedOneWindowArgs& args,
    ...
    const int warp = tid >> 5;
    const int lane = tid & 31;
    const int dv_base = strip * kDvTile + warp * Dv16Schedule::kDvPerWarp;
    const int k_base = lane * Dv16Schedule::kStateColsPerLane;
    const float* state =
...
__device__ __forceinline__ void phase_a_mma(const __half* q_half, const __half* k_half,
    ...
    const bool query = warp >= 2;
    const int token_block = warp & 1;
    const __half* a =
        (query ? q_half : k_half) + token_block * 16 * kQkHalfLd;
    float* out = (query ? y : x) + token_block * 16 * kXyLd;
    mma_macro16<128, false, false>(a, kQkHalfLd, h_half, kQkHalfLd,
...
__device__ __forceinline__ void phase_d_mma(const __half* a_half, const __half* vp_half,
    ...
    if (warp < 2) {
        ...
        mma_macro16<32, false, false>(
            a_half + token_block * 16 * kBridgeLd, kBridgeLd, vp_half, kBridgeLd,
            a_plan.inv * vp_plan.inv, local_output + token_block * 16 * kDvTile,
            kDvTile);
    }
}
...
__device__ __forceinline__ void consume_delta_macro(float (&h)[4][4], const float* macro_base,
    ...
    const int source_warp = k_block - round * 4;
    const float* macro =
...
#pragma unroll
        for (int c = 0; c < 4; ++c) {
            h[r][c] += macro[(row_base + c) * kMacroFp32Ld + dv];
        }
    }
}


__device__ __forceinline__ void process_value_head_chunk_dv16(
...
#pragma unroll
    ...
    for (int round = 0; round < 2; ++round) {
        const int k_block = round * Dv16Schedule::kWarps + warp;
        float* macro =
            delta_macro + warp * 16 * kMacroFp32Ld;
        if (z_plan.mode == BridgeMode::Fp16Mma) {
            mma_macro16<32, false, false>(
                kt_half + k_block * 16 * kKtHalfLd, kKtHalfLd,
                z_half, kKtHalfLd, z_plan.inv, macro, kMacroFp32Ld);
        } else {
            phase_e_simt_macro(kt_half, z, k_block, macro);
        }
