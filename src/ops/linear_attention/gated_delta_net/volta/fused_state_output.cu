...
void fused_state_output_dv32_kernel(FusedOneWindowArgs args) {
    ...
    const int jobs_per_qk_head =
        kGroupSize * fused_dv32_detail::Dv32Schedule::kDvStrips;
    const int job = static_cast<int>(blockIdx.x);
    const int qk_head = job / jobs_per_qk_head;
    const int within_qk = job - qk_head * jobs_per_qk_head;
    const int group_member =
        within_qk / fused_dv32_detail::Dv32Schedule::kDvStrips;
    const int strip =
        within_qk - group_member * fused_dv32_detail::Dv32Schedule::kDvStrips;
    const int value_head = qk_head * kGroupSize + group_member;

    unsigned char* region_a =
        shared_bytes + fused_dv32_detail::Dv32Shared::A;
    unsigned char* region_b =
        shared_bytes + fused_dv32_detail::Dv32Shared::B;
    unsigned char* region_c =
        shared_bytes + fused_dv32_detail::Dv32Shared::C;
    unsigned char* region_d =
        shared_bytes + fused_dv32_detail::Dv32Shared::D;
    unsigned char* region_e =
        shared_bytes + fused_dv32_detail::Dv32Shared::E;

    __half* q_half = reinterpret_cast<__half*>(region_a);
    __half* k_half = reinterpret_cast<__half*>(region_b);

    float h[4][4];
    fused_dv32_detail::load_state(h, args, value_head, strip);

    const int chunks = args.tokens / kChunkSize;
    for (int chunk = 0; chunk < chunks; ++chunk) {
        fused_dv32_detail::stage_qk_chunk(
            args, chunk, qk_head, q_half, k_half);
        fused_dv32_detail::process_value_head_chunk_dv32(
            args, chunk, qk_head, value_head, strip, q_half, k_half,
            region_a, region_c, region_d, region_e, h);
    }

    fused_dv32_detail::store_state(h, args, value_head, strip);
}
