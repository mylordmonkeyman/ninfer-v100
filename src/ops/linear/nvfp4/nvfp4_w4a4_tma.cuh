...
// Token-fast raster places CTAs sharing a weight tile next to each other in
// linear launch order, improving their opportunity to reuse cached weights.
...
template <bool TokenFast, class Geometry, class Schedule, class Epilogue, class OutputPolicy>
__global__
__launch_bounds__(Schedule::kThreads, Schedule::kMinBlocksPerSm) void nvfp4_w4a4_tma_kernel(
    ...
    // With four stages the producer could overwrite a scale pair before its odd
    // consumer completes. The qualified schedules use two or three stages.
