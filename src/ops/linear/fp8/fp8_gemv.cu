...
void launch_exact(const Tensor& x, const Weight& weight, Tensor& out, cudaStream_t stream) {
    ...
    fp8_gemv_kernel<Geometry, Schedule><<<kBlocks, Schedule::kThreads, 0, stream>>>(
        ...
        static_cast<const Scale*>(weight.scales), output);
...
void launch_fp8_decode(const Tensor& x, const Weight& weight, Tensor& out, cudaStream_t stream) {
    ...
    case Fp8Problem::AttnInput:
        launch_exact<Fp8AttnInputGeometry, __nv_bfloat16>(x, weight, out, stream);
        return;
    case Fp8Problem::GdnInput:
        launch_exact<Fp8GdnInputGeometry, __nv_bfloat16>(x, weight, out, stream);
        return;
    case Fp8Problem::MlpGateUp:
        launch_exact<Fp8MlpGateUpGeometry, __nv_bfloat16>(x, weight, out, stream);
        return;
    case Fp8Problem::Vocabulary:
        break;
    case Fp8Problem::Residual6144:
        launch_exact<Fp8Residual6144Geometry, __nv_bfloat16>(x, weight, out, stream);
        return;
    case Fp8Problem::Residual17408:
        launch_exact<Fp8Residual17408Geometry, __nv_bfloat16>(x, weight, out, stream);
        return;
    case Fp8Problem::FlashNextAttnInput:
        launch_exact<Fp8FlashNextAttnInputGeometry, float>(x, weight, out, stream);
        return;
    case Fp8Problem::FlashNextGdnInput:
        launch_exact<Fp8FlashNextGdnInputGeometry, float>(x, weight, out, stream);
        return;
    case Fp8Problem::FlashNextResidual:
        launch_exact<Fp8FlashNextResidualGeometry, float>(x, weight, out, stream);
        return;
