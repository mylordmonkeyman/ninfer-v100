...
int verify_fold_oracle(const FoldProfile profile, std::int32_t width, std::int32_t commit,
                       std::uint32_t seed, int layer, int row,
    ...
    input.state = initial_recurrent_values(actual.size(), seed, layer, row);
    ...
    for (std::int32_t token = 0; token < commit; ++token) {
        ...
        for (std::int32_t head = 0; head < kQkHeads; ++head) {
            ...
            for (std::int32_t dim = 0; dim < kStateDim; ++dim) {
                input.k[(static_cast<std::size_t>(token) * kQkHeads + head) * kStateDim + dim] =
                    bf16_to_f32(key_records[base + dim]);
            }
        }
        for (std::int32_t head = 0; head < profile.value_heads; ++head) {
            ...
            for (std::int32_t dim = 0; dim < kStateDim; ++dim) {
                input.v[(static_cast<std::size_t>(token) * profile.value_heads + head) * kStateDim + dim] =
                    bf16_to_f32(value_records[value_base + dim]);
            }
...
int run_case(const FoldProfile profile, std::int32_t width, std::int32_t rows,
    ...
    for (std::int32_t layer = 0; layer < profile.layers; ++layer) {
        for (std::int32_t row = 0; row < rows; ++row) {
            ...
            // Snapshot bit parity is supplementary. Qualify the complete recurrence directly
            // from represented public records; the oracle does not reproduce FP32 reduction order.
            if (commits[row] > 0 && (layer == 0 || (layer == profile.layers - 1 && row == rows - 1))) {
                failures += verify_fold_oracle(profile, width, commits[row], seed, layer, row,
