...
FakeAssessedPressureTarget FakePressurePlanningSession::assess(FakePressureTargetHandle handle) {
    ...
    if (program_->pressure_assessment_delay_us != 0) {
        // Spin rather than sleep: Windows rounds a 1 ms sleep up to the 15.6 ms timer period,
        // which would exhaust the planner's wall budget after three assessments and turn these
        // cost-shaped tests into timing tests.
