...
int test_structured_tool_output() {
    ...
    FrontendResources owned = resources();
    owned.tokenizer_json = read_file((dir + "/tokenizer.json").c_str());
    owned.tokenizer_config_json = read_file((dir + "/tokenizer_config.json").c_str());
    owned.generation_config_json = read_file((dir + "/generation_config.json").c_str());
    const Frontend frontend = FrontendFactory::create_component(owned);
    ...
    try {
        ...
        auto run = [&](const char* name, auto fn) {
            try {
                int f = fn();
                if (f != 0) {
                    std::cerr << "FAILED: " << name << " (failures=" << f << ")\n";
                }
                return f;
        ...
        int failures = 0;
        failures += run("official_tokenizer_merge", test_official_tokenizer_merge);
        failures += run("bpe_merge_order", test_bpe_merge_order);
        failures += run("boundary_aware_tokenization", test_boundary_aware_tokenization);
        failures += run("literal_added_token_provenance", test_literal_added_token_provenance);
        failures += run("repeated_special_tokens_scan_linearly", test_repeated_special_tokens_scan_linearly);
        failures += run("bounded_tokenizer_prefix", test_bounded_tokenizer_prefix);
        failures += run("context_capacity_guard", test_context_capacity_guard);
        failures += run("official_chat_template", test_official_chat_template);
        failures += run("ordered_instruction_turns", test_ordered_instruction_turns);
        failures += run("assistant_continuation", test_assistant_continuation);
        failures += run("reasoning_effort_chat_template", test_reasoning_effort_chat_template);
        failures += run("rewrite_checkpoint_trace", test_rewrite_checkpoint_trace);
        failures += run("adjacent_tool_message_boundary", test_adjacent_tool_message_boundary);
        failures += run("official_resource_guards", test_official_resource_guards);
        failures += run("invalid_public_part_enums", [&] { return test_invalid_public_part_enums(frontend); });
#if NINFER_BUILD_MEDIA_ENABLED
        failures += run("text_and_image_prepare", [&] { return test_text_and_image_prepare(frontend); });
        failures += run("literal_control_tokens_with_media", test_literal_control_tokens_with_media);
        failures += run("image_resize_rejection_policy", test_image_resize_rejection_policy);
        failures += run("explicit_leading_instruction_cache_boundary", test_explicit_leading_instruction_cache_boundary);
        failures += run("media_admission_uses_aggregate_resources", [&] { return test_media_admission_uses_aggregate_resources(frontend); });
        failures += run("multimodal_prompt_over_removed_32k_cap", [&] { return test_multimodal_prompt_over_removed_32k_cap(frontend); });
        failures += run("attention_pairs_are_diagnostic", [&] { return test_attention_pairs_are_diagnostic(frontend); });
        failures += run("video_prepare", [&] { return test_video_prepare(frontend); });
#endif
        failures += run("cross_round_stop", [&] { return test_cross_round_stop(frontend); });
        failures += run("same_token_stop_priority", [&] { return test_same_token_stop_priority(frontend); });
        failures += run("terminal_flush", [&] { return test_terminal_flush(frontend); });
        failures += run("structured_tool_output", test_structured_tool_output);
        failures += run("reasoning_split", [&] { return test_reasoning_split(frontend); });
        failures += run("thinking_budget_control", [&] { return test_thinking_budget_control(frontend); });
        failures += run("utf8_and_hidden_eos", [&] { return test_utf8_and_hidden_eos(frontend); });
#if NINFER_BUILD_MEDIA_ENABLED
        failures += run("media_cache_reuses_immutable_payload", test_media_cache_reuses_immutable_payload);
        failures += run("media_payload_outlives_frontend_cache", test_media_payload_outlives_frontend_cache);
        failures += run("media_live_bytes_follow_last_payload_reference", test_media_live_bytes_follow_last_payload_reference);
        failures += run("media_cache_singleflight", test_media_cache_singleflight);
        failures += run("media_cache_runs_independent_misses_in_parallel", test_media_cache_runs_independent_misses_in_parallel);
        failures += run("many_images_prepare_in_one_parallel_batch", test_many_images_prepare_in_one_parallel_batch);
        failures += run("media_preparation_cancellation", test_media_preparation_cancellation);
        failures += run("invalid_media_classification", test_invalid_media_classification);
#endif
        failures += run("disabled_vision", test_disabled_vision);
