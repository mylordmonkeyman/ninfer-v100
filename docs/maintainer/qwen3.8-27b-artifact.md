...
### 12.5 Adding DFlash2 to an existing artifact

An existing Qwen3.8-27B base artifact can be extended without its original target checkpoints:

```bash
python3 -m tools.convert.qwen3_8_27b.splice_dflash2 \
  --target /path/to/qwen3_8_27b_nvfp4.ninfer \
  --dflash2-model /path/to/Qwen3.8-27B-DFlash2 \
  --out out/qwen3_8_27b_nvfp4_dflash2.ninfer \
  --device cpu
```

This route accepts either registered Qwen3.8-27B weight profile with its exact base inventory.
It validates tensor descriptors and frontend checksums, copies every base payload unchanged, and
encodes the same 66-object DFlash2 suffix as a full conversion. It writes a separate container and
verifies every destination payload before publishing the output path. Existing outputs and
artifacts that already contain a DFlash2 suffix are rejected. The adjacent `.conversion.json`
records companion provenance and the copied/encoded payload checksums. The original base artifact
remains usable for ordinary decoding and MTP.
