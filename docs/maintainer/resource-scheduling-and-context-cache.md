...
未结算的 Fork 使 Program 暂时无法提供 admission candidate，即使独立请求可行、目标 lane 空闲且
物理容量充足。ResourceManager 将此状态视为 TemporarilyBlocked，在 active transaction 结算后重试；
不能通过 pressure composition 将尚未完成的状态转换误判为可通过回收容量解决的问题。
